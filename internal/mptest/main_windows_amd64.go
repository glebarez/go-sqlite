...
func runScript(tls *libc.TLS, iClient int32, taskId int32, zScript uintptr, zFilename uintptr) { /* mptest.c:872:13: */
	...
	*(*int32)(unsafe.Pointer(bp + 536 /* lineno */)) = 1
	...
	for (libc.AssignInt32(&c, int32(*(*int8)(unsafe.Pointer(zScript + uintptr(ii)))))) != 0 {
		...
		for j = nArg; j < 2; j++ {
			*(*int8)(unsafe.Pointer((bp + 570 /* &azArg[0] */ + uintptr(libc.PostIncInt32(&j, 1))*100) + uintptr(0))) = int8(0)
		}
		...
		if libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1211 /* "sleep" */) == 0 {
			sqlite3.Xsqlite3_sleep(tls, libc.Xatoi(tls, (bp+570 /* &azArg[0] */ +uintptr(0)*100)))
		} else
		...
		if libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1217 /* "exit" */) == 0 {
			var rc int32 = libc.Xatoi(tls, (bp + 570 /* &azArg[0] */ + uintptr(0)*100))
		...
		if (libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1295 /* "glob" */) == 0) || (libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1300 /* "notglob" */) == 0) {
			...
			var zAns uintptr = (zScript + uintptr(ii))
			var zCopy uintptr
			var isGlob int32 = (libc.Bool32(int32(*(*int8)(unsafe.Pointer(bp + 540 /* &zCmd[0] */ + uintptr(0)))) == 'g'))
		...
		if libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1358 /* "source" */) == 0 {
			...
			zNewFile = (bp + 570 /* &azArg[0] */ + uintptr(0)*100)
		...
		if (libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1441 /* "start" */) == 0) && (iClient == 0) {
			var iNewClient int32 = libc.Xatoi(tls, (bp + 570 /* &azArg[0] */ + uintptr(0)*100))
			if iNewClient > 0 {
		...
		if (libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1447 /* "wait" */) == 0) && (iClient == 0) {
			var iTimeout int32
			if nArg >= 2 {
				iTimeout = libc.Xatoi(tls, (bp + 570 /* &azArg[0] */ + uintptr(1)*100))
			} else {
				iTimeout = 10000
			}
			...
			waitForClient(tls, libc.Xatoi(tls, (bp+570 /* &azArg[0] */ +uintptr(0)*100)), iTimeout, bp+770 /* &zError[0] */)
		} else
		...
		if (libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1467 /* "task" */) == 0) && (iClient == 0) {
			var iTarget int32 = libc.Xatoi(tls, (bp + 570 /* &azArg[0] */ + uintptr(0)*100))
			var iEnd int32
			var zTask uintptr
			var zTName uintptr
			...
			if iTarget < 0 {
				...
				if nArg > 1 {
					zTName = sqlite3.Xsqlite3_mprintf(tls, ts+438 /* "%s" */, libc.VaList(bp+392, (bp+570 /* &azArg[0] */ +uintptr(1)*100)))
				} else {
		...
		if libc.Xstrcmp(tls, bp+540 /* &zCmd[0] */, ts+1582 /* "show-sql-errors" */) == 0 {
			g.bIgnoreSqlErrors = func() int32 {
				if nArg >= 1 {
					return libc.BoolInt32(!(booleanValue(tls, (bp+570 /* &azArg[0] */ +uintptr(0)*100)) != 0))
				}
				return 1
			}()
		} else {
...
func main1(tls *libc.TLS, argc int32, argv uintptr) int32 { /* mptest.c:1279:18: */
	...
	*(*int32)(unsafe.Pointer(bp + 400 /* n */)) = (argc - 2)
	...
	if zClient != 0 {
		...
		var nTry int32 = 0
		if g.iTrace > 0 {
			libc.Xprintf(tls, ts+2611 /* "BEGIN: %s" */, libc.VaList(bp+104, *(*uintptr)(unsafe.Pointer(argv + uintptr(0)*8))))
			for i = 1; i < argc; i++ {
	...
	if iClient > 0 {
		if *(*int32)(unsafe.Pointer(bp + 400 /* n */)) > 0 {
			unrecognizedArguments(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(0)*8)), *(*int32)(unsafe.Pointer(bp + 400 /* n */)), (argv + uintptr(2)*8))
		}
		if g.iTrace != 0 {
		...
		*(*uintptr)(unsafe.Pointer(bp + 408 /* zScript */)) = readFile(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(2)*8)))
		for iRep = 1; iRep <= nRep; iRep++ {
			if g.iTrace != 0 {
				logMessage(tls, ts+3467 /* "begin script [%s..." */, libc.VaList(bp+296, *(*uintptr)(unsafe.Pointer(argv + uintptr(2)*8)), iRep))
			}
			runScript(tls, 0, 0, *(*uintptr)(unsafe.Pointer(bp + 408 /* zScript */)), *(*uintptr)(unsafe.Pointer(argv + uintptr(2)*8)))
			if g.iTrace != 0 {
				logMessage(tls, ts+3495 /* "end script [%s] ..." */, libc.VaList(bp+320, *(*uintptr)(unsafe.Pointer(argv + uintptr(2)*8)), iRep))
			}
		}
	...
	if iClient == 0 {
		...
		libc.Xprintf(tls, ts+3662 /* "END: %s" */, libc.VaList(bp+368, *(*uintptr)(unsafe.Pointer(argv + uintptr(0)*8))))
		for i = 1; i < argc; i++ {
