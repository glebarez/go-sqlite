...
func speedtest1_final(tls *libc.TLS) { /* speedtest1.c:416:6: */
	...
	if g.bVerify != 0 {
		...
		HashFinal(tls)
		for i = 0; i < 24; i++ {
			libc.Xprintf(tls, ts+2755 /* "%02x" */, libc.VaList(bp+40, int32(*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 3136 + 259 + uintptr(i))))))
		}
	...
	for sqlite3.Xsqlite3_step(tls, g.pStmt) == SQLITE_ROW {
		...
		for i = 0; i < n; i++ {
			...
			if uint64(g.nResult+len) < uint64(unsafe.Sizeof([3000]uint8{}))-uint64(2) {
				if g.nResult > 0 {
					*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 124 + uintptr(libc.PostIncInt32(&g.nResult, 1)))) = uint8(' ')
				}
